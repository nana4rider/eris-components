...
function ErisComponentsClient(
    ...
    if (!validInstance && options.invalidClientInstanceError)
        throw new ErisComponentsError(
    ...
    ErisClient.on('rawWS', async (packet) => {
        if (packet.t === 'INTERACTION_CREATE') {
            ...
            debug(options, 'WebSocket Event INTERACTION_CREATE emitted.');
    ...
    ErisClient.editComponents = async function (
        ...
        if (components instanceof Button) {
            ...
            return await ErisClient.request(
                body,
                null,
                endpoint,
                'PATCH',
    ...
    ErisClient.request = async function (
        ...
        debug(options, `Request to ${endpoint} using method ${method}.`);

        return await ErisClient.requestHandler
            .request(
                method,
                endpoint,
                true,
            ...
            .catch((error) => {
                debug(options, 'An error occurred while doing request');
